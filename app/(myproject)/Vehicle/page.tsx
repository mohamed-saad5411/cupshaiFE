...
export default function Vehicle() {
  ...
  const slidedata: SlideItem[] = [
    { id: 1, name: 'Vehicle', current: true, logo: Electronicss },
    { id: 2, name: 'Electronics', current: false, logo: automotive },
    { id: 3, name: 'Watches', current: false, logo: watch },
    { id: 4, name: 'Jobs', current: false, logo: job },
    { id: 5, name: 'Watches', current: false, logo: watch },
    { id: 6, name: 'Clothes', current: false, logo: clothes },
    { id: 7, name: 'Furniture', current: false, logo: sofa },
    { id: 8, name: 'Clothes', current: false, logo: clothes },
    { id: 9, name: 'Watches', current: false, logo: watch }
  ]
              ...
              <div key={item.id} className="mb-4 ps-1">
                <Link href={`/${item.name}`} className="slide-item flex items-center justify-start">
                  <div className="flex items-center justify-start">
                    <div className="md:h-[2rem] me-2 h-[2rem] w-[2rem] md:w-[2rem] transition-all duration-400 text-[#666666] hover:bg-black rounded-full flex items-center justify-center">
                      <Image
                        src={item.logo}
                        alt={item.name}
                        className="w-[70%]"
                    ...
                    <p className={item.current ? 'p-slider transition-all text-black duration-400 hover:text-black' : 'text-[#666666]'}>
                      {item.name}
                    </p>
                  </div>
                </Link>
